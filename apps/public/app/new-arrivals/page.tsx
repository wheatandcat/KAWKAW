...
function getNewArrivals(): (Product & {
  ...
  const rng = (i: number) =>
    ((dayOfYear * 7919 + 104729 + i * 1637) % 233280) / 233280;
...
export default function NewArrivalsPage() {
  ...
  const thisWeekItems = filtered.filter(
    (p) => !latestProductIds.has(p.id) && p.daysAgo >= 1 && p.daysAgo <= 7,
  );
  const olderItems = filtered.filter(
    (p) => !latestProductIds.has(p.id) && p.daysAgo > 7,
  );
            ...
            return (
              ...
              >
                <Link
                  ...
                  data-testid={`new-link-${product.id}`}
                >
                ...
                <div className="flex flex-col flex-1 px-3 pb-3 gap-1.5">
                  <Link href={`/product/${product.id}`}>
                    <h3
                      className="text-xs font-medium text-foreground line-clamp-2 cursor-pointer hover:underline"
                      data-testid={`new-name-${product.id}`}
                    >
                      {product.name}
                    </h3>
                  </Link>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1
                ...
                data-testid="text-newarrivals-title"
              >
                新着アイテム
              </h1>
